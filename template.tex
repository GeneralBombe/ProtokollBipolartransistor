...
Die Übersteuerungsgrenze ist die Linie, bei der $U_{CB} = 0\,\mathrm{V}$ gilt. Sie kann als Gerade im Kennlinienfeld eingezeichnet werden, typischerweise parallel zur $U_{CE}$-Achse bei entsprechendem Wert.
\subsubsection{Eingangskennlinie}
\subsubsection{Temperaturverhalten}
...
\paragraph{a) Zeichnen Sie die Arbeitsgerade des Verstärkers durch den Arbeitspunkt I C = 20 mA in das Ausgangskenn-
linienfeld ein. Markieren Sie die Schwingungsbereiche des Kollektorstroms auf der I C-Achse, sowie der
Ausgangsspannung auf der U CE-Achse. Verwenden Sie dazu den in B.2.2 gemessenen Spitze-Tal-Wert der
Ausgangsspannung im verzerrungsfreien Betrieb.}

\paragraph{b) Erklären Sie, warum das Ausgangssignal bei zu großen Eingangsamplituden nicht mehr sinusförmig ist.}
Wenn die Arbeitspunktspannung des Transistor genau zwischen der Versorgungsspannung und dem Bezugspotential lieght, ist der Ausgangssteuerungsbereich am größten. Ist die Eingangsamplitude zu groß, wird das Signal, welches eigentlich über bzw. unterhalb der Versorgungsschienen liegt auf jeweils dessen Spannungsnieveau abgeschnitten, wie in  Abbildung~\ref{fig:supplyrailskrass} zu sehen. 
Um die Abweichungen bei schon deutlich niedrigeren Amplituden zu erklären, ist es hilfreich die Eingangskennlinie in Abbildung \ref{fig:Eingangskennlinie} zu betrachteten. Bei einem Eingangssignal mit einer Amplitude in $U_V$ verhält sich die Verstärkung linear.
