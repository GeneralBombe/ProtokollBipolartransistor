...
Die Gate-Soruce Spannungsquelle wird auf $\SI{660}{\milli\volt}$, $\SI{750}{\milli\volt}$, $\SI{860}{\milli\volt}$ und $\SI{980}{\milli\volt}$ eingestellt, um die Basisströme $\SI{300}{\micro\ampere}$, $\SI{400}{\micro\ampere}$, $\SI{500}{\micro\ampere}$ und $\SI{600}{\micro\ampere}$ festzulegen.
Diese Parameter wurden mit ''.step'' alle in einem Diagramm Dargestellt. Die 
\begin{figure}[H]
	\centering
...
\begin{figure}[H]
	\centering
	\includegraphics[width=1.0\textwidth]{Figures/LTSpiceTime300u.png}
	\caption{Zeitlicher Verlauf von $I_B$ (unten),$U_{CE}$ (mitte) und $I_C$ (oben).}
	\label{fig:300uVerlauf3}
\end{figure}


\subsubsection{Ausschaltzeiten}
Die Messungen haben im Praktikum nicht funktioniert. Daher wurde die Aufgabe mithilfe von \texttt{LTspice} simuliert und eine entsprechende Schaltung geplottet. Der Basisstrom wurde im Bereich von \(300\,\upmu\text{A}\) bis \(600\,\upmu\text{A}\) variiert. Beim Ausschalten betrachten wir die fallende Flanke des Basisstroms \(I_B\).



\subsubsection{Einschaltzeiten}
Beim Einschalten betrachten wir die steigende Flanke des Basisstroms \(I_B\).
...
\paragraph{a)}
Der hier sichtbare Widerstand ist durch die Widerstände in Reihe mit dem Kollektor zurückzuführen. Die kürzeren Kennlinien bei höheren Basisströmen, liegt an dem geringeren Spannungsabfall $U_{CE}$, welche jetzt an $R_C + R_{gen}$ abfällt. 
\[
...
Anschließend wurden folgende Werte entnommen (siehe Abbildung~\ref{fig:arbeitspunkte schaltkonfiguration}):

\begin{table}[H]
\centering
\begin{tabular}{c|c}
$I_B$ (µA) & $U_{CE}$ (in V aus Arbeitsgerade) \\ \hline
600 & 0,2  \\ \hline
500 & 1,35 \\ \hline
400 & 3,0  \\ \hline
300 & 4,8 
...
$I_B$ (µA) & $U_{CE}$ (in V aus Simulation) & $I_C$ (in mA) & $\beta = \frac{I_C}{I_B}$ \\ \hline
600 & 7,8V & 42mA & 70\\ \hline
500 & 7,2V & 35mA & 70\\ \hline
400 & 6,2V & 28mA & 70\\ \hline
300 & 5,8V & 22mA & 70
\end{tabular}
\caption{Auswertung der Arbeitspunkte bei verschiedenen Basisströmen, Simulationsbasiert}
\label{tab:arbeitspunkte}
\end{table}


Die Werte stimmen nicht mit den Vergleichsergebnissen überein, da zur Simulation ein anderer Bipolartransistor genutzt wurde.
...
Für die relevanten Strom und Spannungsverläufe siehe Abbildung \ref{fig:300uVerlauf3}.
\paragraph{a)}
...
Wir messen Ein- und Ausschaltzeiten im Bereich von \( 0{,}9 \cdot I_C \) bis \( 0{,}1 \cdot I_C \).\\


Die Einschaltzeiten  \( t_e \):
...
\includepdf[pages={1}]{simulation.pdf}
